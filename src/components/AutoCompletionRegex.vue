...
function isCheck(text: string) {
  return preOptionsComputed.value.some(v => {
    return v.value == text;
  });
}
...
function isSelected(value: CustomOptionValue) {
  return props.modelValue.split(splitRegex).some(v => v.trim() == value.value);
}
...
const preOptionsComputed = computed(() => {
  ...
  function isStartWith(option: CustomOptionValue) {
    return (
      utils.StringStartWith(option.value, lastChunk) ||
      utils.StringStartWith(option.text, lastChunk)
    );
  }
...
function select(option?: CustomOptionValue) {
  ...
  let r: string[];
  if (inputChunks.every(v => v != option.value)) {
